...
def EquatorialToGalactic(Equatorial):
    ...
    cd     = np.cos(dec)
    sd     = np.sin(dec)
    craminus=np.cos(ra-ragp)
    sraminus=np.sin(ra-ragp)
    b      = np.arcsin(sdecgp*sd+cdecgp*cd*craminus)
    l      = lcp-np.arctan2(cd*sraminus,cdecgp*sd-sdecgp*cd*craminus)
    l[l<0] += 2.*np.pi;    
    ...
    cb         = np.cos(b)
    sb         = np.sin(b)
    slpminus   = np.sin(lcp-l)
    clpminus   = np.cos(lcp-l)
    A11        = (sdecgp*cd-cdecgp*sd*craminus)/cb
    A12        = -cdecgp*sraminus/cb
    A21        = (cdecgp*cd+sdecgp*sd*craminus+sb*clpminus*A11)/slpminus
    A22        = (sdecgp*sraminus+sb*clpminus*A12)/slpminus
    index      = np.where(np.fabs(clpminus) > np.fabs(slpminus))
    A21[index] = (sd[index]*np.sin(ra[index]-ragp)-sb[index]*np.sin(lcp-l[index])*A11[index])/np.cos(lcp-l[index])
    A22[index] = -(np.cos(ra[index]-ragp)+sb[index]*np.sin(lcp-l[index])*A12[index])/np.cos(lcp-l[index])
